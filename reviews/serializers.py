...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Review
        fields = ["id", "movie", "movie_id", "user", "text", "created_at", "updated_at", "score", "rating_score"]
    ...
    def create(self, validated_data):
        ...
        Rating.objects.update_or_create(movie=review.movie, user=review.user, defaults={"score": score})
    ...
    def update(self, instance, validated_data):
        if "score" in validated_data:
            ...
            Rating.objects.update_or_create(movie=instance.movie, user=instance.user, defaults={"score": score})
...
class RatingSerializer(serializers.ModelSerializer):
    ...
    movie = MovieListSerializer(read_only=True)
    movie_id = serializers.IntegerField(write_only=True)
    score = serializers.DecimalField(max_digits=3, decimal_places=1, min_value=Decimal("0.0"), max_value=Decimal("5.0"))

    class Meta:
        model = Rating
        fields = ["id", "movie", "movie_id", "user", "score", "created_at", "updated_at"]
        read_only_fields = ["created_at", "updated_at"]

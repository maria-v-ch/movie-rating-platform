...
class SecurityTests(APITestCase):
    ...
    def test_csrf_protection_on_registration(self):
        ...
        response = self.django_client.post(reverse("users:register"), self.user_data, HTTP_X_CSRFTOKEN=csrf_token)
    ...
    def test_csrf_protection_on_login(self):
        ...
        # Try without CSRF token
        response = self.django_client.post(reverse("users:login"), {"username": "testuser", "password": "testpass123"})
        self.assertEqual(response.status_code, 403)
        ...
        response = self.django_client.post(
            reverse("users:login"), {"username": "testuser", "password": "testpass123"}, HTTP_X_CSRFTOKEN=csrf_token
        )
    ...
    def test_xss_prevention(self):
        ...
        user = User.objects.create_superuser(
            username="xss_test_user", email="xss_test@example.com", password="testpass123"
        )
